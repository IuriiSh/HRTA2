...
-- QUROQ_ABSORBING = 0.0005 antcom - неактуальная спеца. Новая: +4 маны за дохлого врага и +8 за союзника
QUROQ_MANA_FOR_ENEMY_UNIT_DEATH = 4
QUROQ_MANA_FOR_ALLY_UNIT_DEATH = 8
...
function NumberSideToText(side)
  local text
  if side == 0 then text = 'attacker' end        --====  local text
  if side == 1 then text = 'defender' end        --====  return side==0 and 'attacker' or 'defender'
  return text
end
...
--    end
  ...
  -- Система не гибкая, добавляю простую проверку.
...
function CreatureDead(unit)
  ...
  local dead_creature_side = GetUnitSide(unit)
  ...
  for side, hero in {[0]=GetHero(0); GetHero(1)} do
    if GetHeroSkillMastery(hero, 170) > 0 and GetHeroSkillMastery(hero, 15) > 0 and type < 900 and (GetUnitSide(unit) ~= GetUnitSide(hero))  then
      ...
      --print(unit_get_move)
      --print(length(creatures_necro))
      --for i = 0, (length(creatures_necro)-1) do
      --  print(i, ': ',creatures_necro[i])
      --end
 ...
 -- end
  ...
  if IsNamedHero(ally_hero, 'Quroq') or IsNamedHero(enemy_hero, 'Quroq') then
    ...
    --print("We are in Quroq")

    local quroq_side = 0

    if (IsNamedHero(ally_hero, 'Quroq') and dead_creature_side == 0) or (IsNamedHero(enemy_hero, 'Quroq') and dead_creature_side == 1) then quroq_side = 0 else quroq_side = 1 end

    local quraq_max_mana_points = GetUnitMaxManaPoints(GetHero(quroq_side))
    local quraq_current_mana_points = GetUnitManaPoints(GetHero(quroq_side))
    local delta_mana_points = quraq_max_mana_points - quraq_current_mana_points

    if quroq_side == dead_creature_side then -- сдох наш юнит

      if delta_mana_points < QUROQ_MANA_FOR_ALLY_UNIT_DEATH then --мана не выше предела
        SetUnitManaPoints(GetHero(quroq_side), delta_mana_points + GetUnitManaPoints(GetHero(quroq_side))) --до максимума маны
      else
        SetUnitManaPoints(GetHero(quroq_side), QUROQ_MANA_FOR_ALLY_UNIT_DEATH + GetUnitManaPoints(GetHero(quroq_side))) -- всю ману за смерить юнита
      end

    else -- сдох вражеский
    
      if delta_mana_points < QUROQ_MANA_FOR_ENEMY_UNIT_DEATH then --мана не выше предела
        SetUnitManaPoints(GetHero(quroq_side), delta_mana_points + GetUnitManaPoints(GetHero(quroq_side))) --до максимума маны
      else
        SetUnitManaPoints(GetHero(quroq_side), QUROQ_MANA_FOR_ENEMY_UNIT_DEATH + GetUnitManaPoints(GetHero(quroq_side))) -- всю ману за смерить юнита
      end
...
--    last_turn_player = GetUnitSide(unit)
    --tt = tt + 1
    --print("N", tt, "last_turn_player = player_", last_turn_player + 1, "(", last_turn_player, ")")
--  end
...
--    end
  ...
  -- Курак
  -- antcom оставляю старую спецу Курака потомкам для изучения в DeletedSpecs.txt
  -- При повышении уровня крови Курак получал/терял 0000.5 единиц маны за каждую единичку крови всей армии
...
--		end
      ...
      if GetHeroLuck(unit) >= random(10) then
		...
		if GetHeroSkillMastery(unit, 40) > 0 or GetHeroSkillMastery(unit, 146) > 0 then
...
--	print('<color=yellow>Automatic finish disabled')
  ...
  finish = 0
    ...
    if real_creatures_remained[0] == nil or real_creatures_remained[1] == nil then
      finish = 1
      allow_finish = 1
      combatSetPause(1)
      real_creatures_remained[0] = nil
      ...
      if real_creatures_remained[0] == nil and real_creatures_remained[1] == nil then
        ...
        --print('PowerArmyRemainder = 0')
        print('Winner: Player_', looser)
        consoleCmd('game_writelog 0')
      end
    end
    --if real_creatures_remained[0] == nil and real_creatures_remained[1] == nil then
        --print("last_turn_player = ", last_turn_player)
      --consoleCmd('game_writelog 1')
	    --sleep(3)
      --looser = 1 + last_turn_player
      --print('Winner: Player_', looser)
...
function SaveVar()
	local t = {[[@SetGameVar('heroes_battle_info', 'return {]]}
		...
		-- уровень --
		append(t, 'HERO_1_MANA=' .. HERO_1_MANA .. ',')

		-- удача --
		append(t, 'HERO_1_TURN=' .. HERO_1_TURN .. ',')
		
		-- уровень --
		append(t, 'HERO_2_MANA=' .. HERO_2_MANA .. ',')

		-- удача --
		append(t, 'HERO_2_TURN=' .. HERO_2_TURN .. ',')
	...
	SetGameVar('HERO_1_MANA_1', HERO_1_MANA)
	
	print('HERO_1_MANA_1 = ', GetGameVar('HERO_1_MANA_1'))
	
	--interval = GetGameVar("interval", HERO_1_MANA)
